...
class BlockAgent():
    def __init__(self, config):
        ...
        self.map = None
        self.step_counter = 0
        self.loc_goal_dist = config.ENVIRONMENT.LOCAL_GOAL_DIST
        self.loc_goal_change_freq = config.ENVIRONMENT.CHANGE_FREQ
        self.local_goal_succ_dist = config.ENVIRONMENT.LOCAL_GOAL_SUCCESS_DIST
    ...
    def act(self, env, observation, show_info = False):
        ...
        start  = time.time()
        # Get map
        if not isinstance(self.map, type(np.array([1]))):
        ...
        if self.step_counter % self.loc_goal_change_freq == 0 or\
           ...
           marked_map[self.local_goal_on_map[0], self.local_goal_on_map[0]] != 0:
            ...
            (self.local_goal_on_map,
             local_goal_real_relative_vec,
             distance_map) = db.PathPlanner(marked_map,
                                            self.loc_goal_dist,
                                            self.scale, show=False,
                                            return_map=True)
        else:
        ...
        permute_coords = time.time()
        #print(self.gps_local_goal)
        action = self.RL_alg.act(observation) #local policy 0 - forward, 1 - left, 2 -right
        action['action']+=1
        local_policy = time.time()


        if show_info:
            print('-============SPEED INFO ===========')
            print('get_map', get_map - start)
            print('get_market map', get_market_map - get_map)
            print('plan path', plan_path - get_market_map)
            print('permute coords', permute_coords - plan_path)
            print('local policy ', local_policy - permute_coords)
            print('-===================================')
