...
class FMMPlanner():
    ...
    # --------kozub-------------------------
    #draw map with smm distanse, agent(-200), goal(-20) and local_goal(-100)
    def get_fmm_path(self, state):
        import numpy as np
        #print('---> goal coordinates:', self.goal)
        #print('---> aget-goal dist:', np.linalg.norm(np.array(state[:2])-np.array(self.goal)))
        goal_dist = np.copy(self.fmm_dist)

        X = np.linspace(-1, 1, goal_dist.shape[0])
        Y = np.linspace(-1, 1, goal_dist.shape[0])
        X, Y = np.meshgrid(X, Y)

        tmp_state = np.array([int(state[1]), int(state[0])], dtype='int32')
        path = [tmp_state]

        while goal_dist[tmp_state[0], tmp_state[1]] > 5:
            near_dist = np.array(goal_dist[tmp_state[0] - 1:tmp_state[0] + 2, tmp_state[1] - 1:tmp_state[1] + 2])
            tmp = np.argmin(near_dist)

            tmp = np.array([tmp // 3, tmp % 3])
            tmp_state = tmp_state + (tmp - [1, 1])
            path.append(tmp_state)

        local_goal = self.get_next_local_goal(state, goal_range=10)
        #print('When drawing map, local goal is:', local_goal)

        for st in path:
            goal_dist[st[0]-1:st[0]+1, st[1]-1:st[1]+1] = -20
        #goal_dist[goal_dist <= 5] = -20
        goal_dist[self.goal[1]-4:self.goal[1]+4, self.goal[0]-4:self.goal[0]+4] = -20
        goal_dist[local_goal[1]- 2:local_goal[1]+ 2, local_goal[0] - 2: local_goal[0]+ 2] = -100
        goal_dist[int(state[1]) - 2:int(state[1]) + 2, int(state[0]) - 2: int(state[0]) + 2] = -200
        #print('state:', state, 'goal:', self.goal, 'local goal:', local_goal)
        plt.contourf(X, Y, goal_dist)
        plt.colorbar()
        plt.show()
        cv2.waitKey(0)

    def get_next_local_goal(self, state, goal_range=5):
        #using fmm distnanse to find local goal on fmm-path
        import numpy as np

        goal_dist = np.copy(self.fmm_dist)

        #we start from current state and find best map-sell around the tmp_state
        tmp_state = np.array([int(state[1]), int(state[0])], dtype='int32')
