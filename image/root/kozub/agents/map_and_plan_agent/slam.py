...
class DepthMapperAndPlanner(object):
  ...
  def plan_path(self, goal_loc, get_local_goal = False):
    state = self.current_loc*1.
    state[:2] = state[:2]/self.resolution

    #my code
    print('----> plan path state:', state)
    #-------
    ...
    #My code
    if get_local_goal:
      return planner.get_next_local_goal(state, goal_range= 25)
    #end of my code
  ...
  def _act(self, i, obs, done):
    ...
    if self.using_local_policy:
      ...
      if np.any(self.local_goal == None) or np.linalg.norm(self.current_loc[:2]/self.resolution - self.local_goal) < 5:
      ...
      angle = np.sign(delta[1])*np.arccos(delta[0]/np.linalg.norm(delta))
      relative_local = np.array([np.linalg.norm(delta), angle - self.current_loc[2]])
