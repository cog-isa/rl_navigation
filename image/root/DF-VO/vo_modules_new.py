...
from tool.evaluation.tum_tool.pose_evaluation_utils import rot2quat, quat2mat
...
class VisualOdometry():
    def __init__(self, cfg):
        ...
        self.pos = {'pred': [0, 0, 0], 'pred0': [0, 0, 0],
                    'gt': [0, 0, 0], 'gt0': [0, 0, 0]}
        
        self.rot = {'pred': 0, 'pred0': 0,
                    'gt': 0, 'gt0': 0}
        
        self.turn = {'pred': 0, 'gt': 0}
        self.forw = {'pred': 0, 'gt': 0}
    ...
    def setup(self):
        ...
        self.depth_available = depth_src_cases[self.cfg.depth.depth_src]
        if self.depth_available:
            print('==> Precomputed depths: depth network not active')
    ...
    def tracking_hybrid(self):
        ...
        if self.tracking_stage == 0:
            ...
            self.global_poses[0] = SE3(self.gt_poses[self.cur_data['id']])
            
            # initial readable
            pos_start = self.cur_data['pose'].t[:,0]
            rot_start = R.from_matrix(self.cur_data['pose'].R).as_rotvec()[1]
            self.pos['pred'] = pos_start
            self.rot['pred'] = rot_start
            self.poss['pred'].append(pos_start[2::-2])
            self.rots['pred'].append(rot_start)
            if self.cfg.available.gt_pose:
                self.pos['gt'] = pos_start
                self.rot['gt'] = rot_start
                self.poss['gt'].append(pos_start[2::-2])
                self.rots['gt'].append(rot_start)
        ...
        # Second to last frames
            ...
            # update global poses (readable)
            # -- record last pos and rot
            self.pos['pred0'] = self.pos['pred']
            self.rot['pred0'] = self.rot['pred']
            if self.cfg.available.gt_pose:
                self.pos['gt0'] = self.pos['gt']
                self.rot['gt0'] = self.rot['gt']
            
            # -- get new ground truth pos and rot
            if self.cfg.available.gt_pose:
                P = self.gt_poses[self.cur_data['id']]
                self.pos['gt'] = P[:3,3]
                self.rot['gt'] = R.from_matrix(P[:3,:3]).as_rotvec()[1]
            
            # -- get new predicted pos and rot
            self.pos['pred'] = self.cur_data['pose'].t[:,0]
            self.rot['pred'] = R.from_matrix(self.cur_data['pose'].R).as_rotvec()[1]
            
            # -- calculate step forward and turn size
            self.turn['pred'] = self.rot['pred'] - self.rot['pred0']
            self.forw['pred'] = np.sqrt(((self.pos['pred'][0::2] - self.pos['pred0'][0::2])**2).sum())
            if self.cfg.available.gt_pose:
                self.turn['gt'] = self.rot['gt'] - self.rot['gt0']
                self.forw['gt'] = np.sqrt(((self.pos['gt'] - self.pos['gt0'])**2).sum())
            
            # -- calculate new positions
            self.poss['pred'].append([self.poss['pred'][-1][0] - np.cos(self.rots['pred'][-1]) * self.forw['pred'] * 10,
                                      self.poss['pred'][-1][1] - np.sin(self.rots['pred'][-1]) * self.forw['pred'] * 10])
            if self.cfg.available.gt_pose:
                self.poss['gt'].append([self.poss['gt'][-1][0] - np.cos(self.rots['gt'][-1]) * self.forw['gt'],
                                        self.poss['gt'][-1][1] - np.sin(self.rots['gt'][-1]) * self.forw['gt']])

            # -- calculate new rotations
            self.rots['pred'].append(self.rots['pred'][-1] - self.turn['pred'])
            if self.cfg.available.gt_pose:
                self.rots['gt'].append(self.rots['gt'][-1] + self.turn['gt'])
    ...
    def step(self, image, depth=None, gt_pose=None):
        """
            image -> np.array of size [h, w, 3]
            depth -> if available then np.array of size [h, w]
            gt_pose -> if available then array [tx, ty, tz, qx, qy, qz, qw]
        """
        # if not self.cur_data['id']:
            # if need smth at start
        
        # read img
        # add preprocess ???
        self.cur_data['img'] = image
        img_h, img_w, _ = image_shape(image)
        
        # read/compute and preprocess depth
        if self.depth_available:
            self.cur_data['raw_depth'] = depth / 255
        else:
            start_time = time()
            self.cur_data['raw_depth'] = self.deep_models['depth'].inference(img=self.cur_data['img'])
        ...
        if self.cfg.available.gt_pose:
            tx, ty, tz, qx, qy, qz, qw = gt_pose
            P = np.eye(4)
            P[:3, :3] = quat2mat([qx, qy, qz, qw])
            P[:3, 3] = np.asarray([tx, ty, tz])
            
            self.gt_poses[self.cur_data['id']] = P
        ...
        self.ref_data, self.cur_data = self.update_ref_data(self.ref_data,
                                                            self.cur_data,
                                                            self.window_size,
                                                            self.keyframe_step)
        ...
        if self.cfg.available.gt_pose:
            return self.poss['pred'][-1], self.rots['pred'][-1],\
                   self.poss['gt'][-1], self.rots['gt'][-1]
        else:
            return self.poss['pred'][-1], self.rots['pred'][-1]
        
        
    def draw_traj(self,gt_pose=None):
        fig, ax = plt.subplots(figsize=(10, 10))
        if gt_pose is not None:
            plt.plot(np.array(gt_pose)[:,0], np.array(gt_pose)[:,1], 'g.-', label='original')
        plt.plot(np.array(self.poss['pred'])[:,0], np.array(self.poss['pred'])[:,1], 'r.-', label='predicted')
        plt.legend()
        plt.show()