...
class VisualOdometry():
    def __init__(self, cfg):
        ...
        #with open(os.path.join(cfg.directory.gt_pose_dir, cfg.seq, 'groundtruth.txt'), 'r') as f:
        #    arr = [float(el) for el in f.readline().split()[1:]]
        #    mat = np.eye(4)
        #    mat[:3,:3] = quat2mat([arr[-1], *arr[-4:-1]])
        #    mat[:3,3] = arr[:3]
        
        # mat = np.eye(4)
        # mat[:3,:3] = quat2mat([1.,0.,1.,0.])
        
        # predicted global poses
        self.global_poses = {}
        # self.global_poses = {0: SE3(mat)}
    ...
    def synchronize_rgbd_pose_pairs(self):
        ...
        rgb_d_pose_pair = {}
        # KITTI 
        if self.cfg.dataset == "kitti" or "tum" in self.cfg.dataset:
