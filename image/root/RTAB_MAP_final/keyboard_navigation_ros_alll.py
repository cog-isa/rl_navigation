...
def main():
    ...
    config.SIMULATOR.FORWARD_STEP_SIZE = 0.1025
    ...
    env = env_class(config=config, dataset=dataset, agent=agent)
...
def run_episode(env,agent,config):
   ...
   # follower.mode = mode
    ...
    i=0
    top_down_map = None
    env.agent.startx, env.agent.starty, env.agent.startz, = env.current_episode.start_position[2], env.current_episode.start_position[0], env.current_episode.start_position[1]
    ...
    while not done:
        ...
        #best_action = 3
        act = best_action
        observations, rew, done, info = env.step(action=act)
        print(env.obs['pointgoal'],'\t','obs pointgoal')
        print(env.obs['gps'],'\t','obs_gps')
        print(env.agent.posx,env.agent.posy,env.agent.posz,'\t','agent pos_x_y_z')
        print(env.agent.trux,env.agent.truy,env.agent.truz,'\t','agent true_x_y_z')
        print(env.agent.goalx,env.agent.goaly,'\t','agent goal_x_y')
        print(env.agent.startx,env.agent.starty,env.agent.startz,'\t','agent start_x_y_z')
        print(env.agent.slam_start_x, env.agent.slam_start_y, env.agent.slam_start_angle,'\t','agent slam_x_y_angle')
        #print(i,'\t',done,'\t',act)
        i+=1
        
       
def publisher_ros(env,agent):
    
    cvbridge = CvBridge()
      
    image_publisher = rospy.Publisher('/habitat/rgb/image', Image, latch=True, queue_size=100)
