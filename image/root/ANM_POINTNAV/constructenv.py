...
def make_env_fn(args, config: Config, env_class: Type[Union[Env, RLEnv]], rank: int) -> Union[Env, RLEnv]:
...
def construct_envs(args, config: Config, env_class: Type[Union[Env, RLEnv]]) -> VectorEnv:
    ...
    argss = [args for i in range(num_processes)]
    envs = VectorEnv(
        make_env_fn=make_env_fn,
        env_fn_args=tuple(tuple(zip(argss, configs, env_classes, range(num_processes)))))
    return envs