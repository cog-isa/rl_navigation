...
class DDPPOTrainer(PPOTrainer):
    ...
    def train_init(self) -> None:
        ...
        self.envs = construct_envs(
            self.args, self.config, get_env_class(self.config.ENV_NAME)
        )
    ...
    def train(self) -> None:
        ...
        args = self.args
        # slam
        self.nslam_module = Neural_SLAM_Module(args).to(args.device)
        self.slam_optimizer = get_optimizer(self.nslam_module.parameters(),
                                       args.slam_optimizer)
        # Storage
        self.slam_memory = FIFOMemory(args.slam_batch_size)
        ...
        local_map = torch.zeros(num_scenes, 4, local_w, local_h).float().to(args.device)
        ...
        obs_new = torch.from_numpy(np.array([observations[i]['rgb'] for i in range(args.num_processes)])).float().to(args.device)

        _, _, local_map[:, 0, :, :], local_map[:, 1, :, :], _, local_pose = \
            self.nslam_module(obs_new, obs_new, poses, local_map[:, 0, :, :],
      ...
      #  return 0
        ...
        with (TensorboardWriter(self.config.TENSORBOARD_DIR, flush_secs=self.flush_secs)
            ...
            for update in range(start_update, self.config.NUM_UPDATES):
                ...
                for step in range(ppo_cfg.num_steps):
                    #print('!!!!!!!!!!!!!!!!!STEP: ', step, ppo_cfg.num_steps)
                    time_start = time.time()
                    (delta_pth_time,delta_env_time,delta_steps) = self._collect_rollout_step(rollouts, current_episode_reward, running_episode_stats, self.slam_memory, self.slam_optimizer, self.nslam_module,args)
                    time_end = time.time()
                    print('TIME FOR FULL STEP: ',time_end-time_start)
                    ...
                    if (step >= ppo_cfg.num_steps * self.SHORT_ROLLOUT_THRESHOLD) \
                        and int(num_rollouts_done_store.get("num_done")) > (self.config.RL.DDPPO.sync_frac * self.world_size):
                        print('BREAKED')
                        break
