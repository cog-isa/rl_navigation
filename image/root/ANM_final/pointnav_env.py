...
@baseline_registry.register_env(name="Pointnav_Env")
class Pointnav_Env(habitat.RLEnv):

    def __init__(self, argss, rank, config_env, dataset):
        args = argss
        if args.visualize:
            plt.ion()
        ...
        self.follower = ShortestPathFollower(self.habitat_env.sim, 0.36, False)
        ...
        self.local_goal_x_y = None
        self.traversible = None
        self.local_map_frame = None
        self.start_240_240 = None
        self.goal_240_240 = None
    ...
    def reset(self):
     ...
     #   while self.explorable_map is None:
        ...
        fp_proj, self.map, fp_explored, self.explored_map = \
            self.mapper.update_map(depth1, mapper_gt_pose)
        ...
        xx,yy = obs['pointgoal']
        self.goalx, self.goaly = inverse_transform(-xx, yy, 0, 0, np.pi)
        xdif,ydif = self.trux-self.goalx, self.truy-self.goaly
        
        self.start_240_240 = [120.,120.]
        self.goal_240_240 = [self.goalx-120, self.goaly-120]
        self.stg = [120.,120]
        

        return {'rgb':state, 'depth':depth, 'pos':np.array([xdif,ydif])}, self.info
    ...
    def step(self, *args, **kwargs):
        ...
        #self.publish_true_path(obs['agent_position'])
        ...
        return {'rgb':state, 'depth':depth, 'pos':np.array([xdif,ydif])}, rew, done, self.info
    ...
    #        self.visited_gt[x, y] = 1
        ...
        self.start_240_240 = start
        self.goal_240_240 = goal
        # Get short-term goal
     ...
     #       gt_action = 0
     #   else:
     #       gt_action = self._get_gt_action(self.map, start,
     #                                       [int(stg[1]), int(stg[0])],
     #                                       planning_window, start_o)
        ...
        best_action = self.follower.get_next_action(self.habitat_env.current_episode.goals[0].position)
        if best_action is None:
            best_action = int(0)
        if self.timestep>195:
            best_action = int(0)
            
        action_return = best_action#gt_action
    
        output = np.zeros((args.goals_size + 1))

        output[0] = int((relative_angle%360.)/5.)
        output[1] = discretize(relative_dist)    #relative_dist
        output[2] = action_return   #gt_action
    ...
    def _get_stg(self, grid, explored, start, goal, planning_window):
        ...
        self.local_map_frame = (x1,x2,y1,y2)
        ...
        if goal[0]-2 > y1 and goal[0]+3 < y2\
            and goal[1]-2 > x1 and goal[1]+3 < x2:
            traversible[int(goal[0]-y1)-2:int(goal[0]-y1)+3,
                    int(goal[1]-x1)-2:int(goal[1]-x1)+3] = 1
        else:
            goal[0] = min(max(y1, goal[0]), y2)
            goal[1] = min(max(x1, goal[1]), x2)
        ...
        self.goal_to_planer = goal
        
        reachable = planner.set_goal([goal[0]-y1+1, goal[1]-x1+1])

        stg_x, stg_y = start[0] - y1 + 1, start[1] - x1 + 1
        for i in range(self.args.short_goal_dist):
            #print([stg_y, stg_x],[goal[0]-y1+1, goal[1]-x1+1],traversible.shape,' START GOAL TRAV_SHAPE')
            stg_x, stg_y, replan = planner.get_short_term_goal([stg_y, stg_x])
            self.local_goal_x_y=np.copy([stg_y, stg_x])
        if replan:
            stg_x, stg_y = start[0], start[1]
        else:
            stg_x, stg_y = stg_y + y1 - 1, stg_x + x1 - 1
    ...
    def _get_gt_action(self, grid, start, goal, planning_window, start_o):
        ...
        while not path_found:
            ...
            for i in range(1):
                #print([stg_x_gt, stg_y_gt],' START_POS')
