...
#             nn.ConvTranspose2d(16, 2, (4, 4), stride=(2, 2), padding=(1, 1)),
            nn.ConvTranspose2d(16, 3, (4, 4), stride=(2, 2), padding=(1, 1)), # Add new layer on map for semantic data
        ]))
    ...
    def forward(self, obs_last, obs, poses, maps, explored, current_poses, semantic_obs, semantic_maps,
            build_maps=True):
        ...
        semantic_resnet_output = self.resnet_l5(semantic_obs)
        semantic_conv_output = self.conv(semantic_resnet_output)
        
        union_conv_output = nn.cat((conv_output, semantic_conv_output))

        proj1 = nn.ReLU()(self.proj1(
                          union_conv_output.view(-1, self.conv_output_size)))
        if self.dropout > 0:
        ...
        with torch.no_grad():
            ...
            semantic_resnet_output = self.resnet_l5(semantic_last_obs)
            semantic_conv_output = self.conv(semantic_resnet_output)
        ...
        if build_maps:
            ...
            with torch.no_grad():
                ...
                explored2 = torch.cat((explored.unsqueeze(1),
                                       translated[:, 1:2, :, :]), 1)
                semantic_maps2 = torch.cat((semantic_maps.unsqueeze(1), translated[:, 2:, :, :]), 1)

                map_pred = self.pool(maps2).squeeze(1)
                exp_pred = self.pool(explored2).squeeze(1)
                semantic_pred = self.pool(semantic_maps2).squeeze(1)
        ...
        return proj_pred, fp_exp_pred, fp_sem_pred, map_pred, exp_pred, semantic_pred,\
