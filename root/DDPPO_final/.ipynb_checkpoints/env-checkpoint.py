...
class MyRLEnvNew(habitat.RLEnv):
    ...
    def reset(self):
        ...
        self.obs['pos'] = np.array([xdif,ydif])
        ...
        observation['depth'] = np.copy(res(observation['depth']))[:,:,np.newaxis]
    ...
    def step(self, *args, **kwargs):
        ...
        for i in range(self._rl_config.FRAMESKIP):
            ...
            self.curr_loc_gt = pu.get_new_pose(self.curr_loc_gt,
                               (dx_gt, dy_gt, do_gt))
            self.trux = self.curr_loc_gt[0]-12
            self.truy = self.curr_loc_gt[1]-12
            xdif,ydif = self.trux-self.goalx, self.truy-self.goaly
