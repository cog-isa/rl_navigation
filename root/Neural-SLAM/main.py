...
def main():
    ...
    explored_ratios = deque(maxlen=1000)
...
#                 print([infos[env_idx].keys() for env_idx in range(num_scenes)])
#                 print([infos[env_idx]['exp_reward'] for env_idx in range(num_scenes)])
            ...
            if total_num_steps % args.log_interval == 0:
                ...
                if len(g_episode_rewards) > 0:
                    ...
                    wandb.log({'Global step mean': np.mean(per_step_g_rewards),
                               'Global eps mean': np.mean(g_episode_rewards),
                               'Explored ratio mean': np.mean(explored_ratios)})
                ...
                if args.train_local and len(l_action_losses) > 0:
                    ...
                    wandb.log({'Local Loss': np.mean(l_action_losses)})
                ...
                if args.train_global and len(g_value_losses) > 0:
                    ...
                    wandb.log({'Global Loss value': np.mean(g_value_losses),
                               'Global Loss action': np.mean(g_action_losses)})
                ...
                if args.train_slam and len(costs) > 0:
                    ...
                    wandb.log({'SLAM Loss proj': np.mean(costs),
                               'SLAM Loss exp': np.mean(exp_costs),
                               'SLAM Loss pose': np.mean(pose_costs),})
                
                wandb.log({'FPS': int(total_num_steps * num_scenes / (end - start)),
                           'time_steps': total_num_steps * num_scenes})
