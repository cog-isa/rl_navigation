...
#         config_env.freeze()
    ...
    def reset(self):
        ...
        semantic_obs = self.prepare_semantic_observation(obs['semantic'])
        fp_proj, self.map, fp_explored, self.explored_map, fp_semantic, self.semantic_map = \
            self.mapper.update_map(depth, mapper_gt_pose, semantic_obs)
    ...
    def step(self, action):
        ...
        self.info['explored_ratio'] = self.get_explored_ratio()
        self.info = {**self.info, **obs, **info}
...
#         print(np.sum(curr_explored) / np.sum(self.explorable_map))
