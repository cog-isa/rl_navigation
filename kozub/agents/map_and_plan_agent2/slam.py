...
class DepthMapperAndPlanner(object):
  def __init__(self, dt=10, camera_height=125., upper_lim=150., map_size_cm=6000, out_dir=None,
      ...
      recover_on_collision=False, fix_thrashing=False, goal_f=1.1, point_cnt=2, using_local_policy = False):
    ...
    #-------my code----------
    self.using_local_policy = using_local_policy
    if self.using_local_policy:
      self.local_agent = PPO(learning_rate=0.001, gamma=0.99, eps_clip=0.1)
  ...
  def plan_path(self, goal_loc, get_local_goal = False, step_num = -1):
    ...
    #My code
    if step_num % 10 == 0:
      planner.get_fmm_path(state)
    if get_local_goal:
      return planner.get_next_local_goal(state, goal_range= 10)
  ...
  def act(self, observations, step_num):
    if self.RESET:
      self.RESET = False
      return self._act(step_num, observations, True)
    else:
      return self._act(step_num, observations, False)
  ...
  def _act(self, i, obs, done):
    ...
    act, act_seq = self.plan_path(self.goal_loc, step_num=i)
    ...
    if self.using_local_policy:
      ...
      #Chose local goal if it unset or chose new if we achieve it
      #if np.any(self.local_goal == None) or\
      #   np.linalg.norm(self.current_loc[:2]/self.resolution - self.local_goal) < 5 or\
      #   drift:
      #  print('change_local_goal')
      self.local_goal = self.plan_path(self.goal_loc, get_local_goal=True)

      delta =  self.local_goal -  self.current_loc[:2]/self.resolution
      angle = np.sign(delta[1])*np.arccos(delta[0]/np.linalg.norm(delta)) - self.current_loc[2]
      angle = angle % (2*np.pi)
      if angle > np.pi:
        angle = angle - 2*np.pi

      relative_local = np.array([np.linalg.norm(delta), angle])

      print('----> agent location: ', self.current_loc[:2]/self.resolution, self.current_loc[2])
      print('----> local goal', self.local_goal, relative_local)
      print('--->', 'probability of action:', self.local_agent.act(rgb, relative_local)[0])
      print('PPO agent chose action:', self.local_agent.act(rgb, relative_local)[1])
    # ---------------------------------------------------------------------------
